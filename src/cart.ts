// cart.ts - Модуль для работы с корзиной
import { escapeHtml, escapeAttr, safeFetch, showLoadingIndicator, hideLoadingIndicator, showToast, closeAllModals } from './utils';
import type { CartItem, Product } from './types';
import type { ShopInstance } from './products';

export class CartModule {
    private shop: ShopInstance & { isMobile: boolean; profileModule?: any };

    constructor(shop: ShopInstance & { isMobile: boolean; profileModule?: any }) {
        this.shop = shop;
    }
    ...
    addToCart(id: string): void {
        ...
        const existing = this.shop.cart.find((i: CartItem) => i.id === id);
    ...
    changeQuantity(id: string, delta: number): void {
        const item = this.shop.cart.find((i: CartItem) => i.id === id);
        if (!item) return;
    ...
    removeFromCart(id: string): void {
        this.shop.cart = this.shop.cart.filter((i: CartItem) => i.id !== id);
        this.saveCart();
        this.updateCartInfo();
        this.renderCart();
    ...
    updateCartInfo(): void {
        ...
        const total = this.shop.cart.reduce((sum: number, item: CartItem) => sum + item.quantity, 0);
    ...
    renderCart(): void {
        ...
        this.shop.cart.forEach((item: CartItem) => {
            ...
            img.width = 70;
            img.height = 70;
            img.onerror = function(this: HTMLImageElement) { this.src = 'https://via.placeholder.com/80'; };
            ...
            p.textContent = `${parseFloat(item.price.toString()).toFixed(2)} ₽ × ${item.quantity} = ${itemTotal.toFixed(2)} ₽`;
        ...
        if (cartItemsContainer) {
            ...
            cartItemsContainer.addEventListener(eventType, (e: Event) => {
    ...
    showMobileAddressPrompt(): Promise<string | null> {
        return new Promise((resolve) => {
            ...
            setTimeout(() => {
                const inputEl = modal.querySelector('#mobile-address-input') as HTMLInputElement;
    ...
    async processOrder(shippingAddress: string): Promise<void> {
        try {
            ...
            const orderData = {
                items: this.shop.cart.map((item: CartItem) => ({
                    id: item.id,
