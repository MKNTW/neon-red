// products.ts - Модуль для работы с товарами
...
export class ProductsModule {
    ...
    constructor(shop: ShopInstance) {
        this.shop = shop;
    }

    renderSkeletonProducts(count: number = 6): void {
    ...
    async loadProducts(page: number = 1, useCache: boolean = true): Promise<void> {
        const productsContainer = document.getElementById('products');
        if (!productsContainer) return;
        ...
        try {
            ...
            if (data.products) {
                ...
                this.shop.currentPage = data.currentPage || page;
                this.shop.totalPages = data.totalPages || 1;
                this.shop.totalProducts = data.totalProducts || data.products.length;
            } else if (Array.isArray(data)) {
                this.shop.products = data as Product[];
    ...
    renderProducts(): void {
        ...
        this.shop.products.forEach((product, index) => {
            ...
            price.textContent = `${parseFloat(product.price.toString()).toFixed(2)} ₽`;
            ...
            addBtn.setAttribute('aria-disabled', product.quantity === 0 ? 'true' : 'false');
